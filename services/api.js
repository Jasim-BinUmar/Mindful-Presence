...
export const api = {
  ...
  auth: {
     ...
     * Backward compatible - now sends OTP instead of directly creating user
     * @param {Object} userData - User registration data (name, email, password, phoneNumber?, organizationId?)
     * @returns {Object} Response with success, message, expiresIn, requiresVerification
     */
    register: async (userData) => {
      ...
      // Note: This endpoint now sends OTP, doesn't create user directly
      // User will be created after OTP verification via registerVerifyOtp
      // No tokens are returned here, only OTP sent confirmation
     ...
     */
    registerVerifyOtp: async (verificationData) => {
      const response = await fetch(endpoints.auth.registerVerifyOtp, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
     ...
     */
    registerResendOtp: async (resendData) => {
      const response = await fetch(endpoints.auth.registerResendOtp, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
