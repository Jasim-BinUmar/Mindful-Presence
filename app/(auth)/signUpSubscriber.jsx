...
const SignUpSubscriber = () => {
    ...
    const handleSignUp = async () => {
        ...
        try {
            console.log("Registering user with email:", trimmedEmail);
            ...
            if (response.success) {
                Alert.alert('Success', 'Account created successfully!', [
                    {
                        text: 'OK',
                        onPress: () => router.push('/(auth)/successfulReg')
                    }
                ]);
            } else {
                // Show detailed validation errors if available
                let errorMessage = response.message || 'Unable to create account';
                if (response.errors && Array.isArray(response.errors)) {
                    errorMessage = response.errors.map(err => err.msg || err.message || err).join('\n');
